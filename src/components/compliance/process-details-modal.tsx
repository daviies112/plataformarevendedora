...
export function ProcessDetailsModal({ check, open, onOpenChange }: ProcessDetailsModalProps) {
  ...
  // Defensive parsing: ensure processData is always an object with safe defaults
  // Prevents: "Cannot read properties of undefined (reading 'TotalLawsuitsAsAuthor')"
  const processData = useMemo(() => {
    const data = result?.Processes;
    if (!data || typeof data !== 'object') {
      return {
        ...
        FirstLawsuitDate: null,
        LastLawsuitDate: null,
  ...
  // Defensive: ensure basicDataPayload is always safe object with defaults
  const basicDataPayload = useMemo(() => {
    const data = payload?._basic_data?.Result?.[0]?.BasicData;
    if (!data || typeof data !== 'object') {
      return {
        Name: null,
        TaxIdStatus: null,
        TaxIdStatusDate: null,
        BirthDate: null,
        Age: null,
        Gender: null,
        MotherName: null,
  ...
  // Defensive: ensure collectionsPayload is always safe object with defaults
  const collectionsPayload = useMemo(() => {
    const data = payload?._collections?.Result?.[0]?.Collections;
    if (!data || typeof data !== 'object') {
      return {
        HasActiveCollections: false,
        TotalOccurrences: 0,
        Last3Months: 0,
        Last12Months: 0,
        ConsecutiveMonths: 0,
