...
function getJwtSecret(): string {
  ...
  if (!secret) {
    throw new Error('[multiTenantAuth] ❌ CRÍTICO: JWT_SECRET não configurado no .env');
  }
  return secret;
}
...
async function handleDirectLogin(req: Request, res: Response, adminData: any, email: string, senha: string) {
  ...
  // 🔐 MULTITENANT: tenant_id canônico vem de admin_users.tenant_id
  // NUNCA gerar baseado no email — deve ser o mesmo em toda a plataforma
  const tenantId = adminData.tenant_id || adminData.id;
  const userName = adminData.nome || adminData.name || adminData.company_name || email.split('@')[0];

  if (!adminData.tenant_id) {
    console.warn(`⚠️ [AUTH] admin_users.tenant_id ausente para ${email} — usando id como fallback`);
  }
  ...
  supabaseOwner?.from('admin_users').update({ last_login: new Date().toISOString() })
    .eq('id', adminData.id).then(undefined, console.error);
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    // 🔐 MULTITENANT: usar admin.tenant_id canônico, não admin.id
    // CRUCIAL: deve ser o mesmo tenant_id que a plataformacompleta-store usa
    const resolvedTenantId = admin.tenant_id || admin.id;
    if (!admin.tenant_id) {
      console.warn(`⚠️ [AUTH] admin.tenant_id ausente para ${admin.email} — usando admin.id como fallback`);
    }
    ...
    req.session.tenantId = resolvedTenantId;
