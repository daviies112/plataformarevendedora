...
function getJwtSecret(): string {
  ...
  if (!secret) {
    throw new Error('[resellerAuth] ❌ CRÍTICO: JWT_SECRET não configurado no .env');
  }
  return secret;
}
...
interface ResellerTokenPayload {
  ...
  userName: string;
  userRole: 'reseller';
  resellerId: string;   // 🔐 UUID único da revendedora (derivado do CPF)
  tenantId: string | null;  // 🔐 NÍVEL 1: empresa/admin
  cpfNormalizado?: string;   // CPF sem formatação (chave de identidade)
  companySlug?: string;
  comissao: number;
  projectName?: string;
}
...
function verifyResellerToken(token: string): ResellerTokenPayload | null {
  try {
    return jwt.verify(token, getJwtSecret()) as ResellerTokenPayload;
  } catch {
    return null;
  }
}
...
// GET /api/reseller/test-db - Testar conexão com Postgres Local
router.get('/test-db', async (_req: Request, res: Response) => {
  try {
    ...
    const revCount = await pool.query('SELECT count(*) FROM revendedoras');

    res.json({
      status: 'connected',
      message: 'Conexão com Postgres Local OK',
      database: result.rows[0],
      tablesCount: tablesCount.rowCount,
      revendedorasCount: revCount.rows[0].count
    });
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const revendedora = revendedoras?.find((r: any) => {
      const cpfDb = r.cpf?.replace(/\D/g, '');
      const match = cpfDb === cpfNormalizado && ['ativo', 'pendente'].includes(r.status);
      console.log('[NEXUS] Comparando CPF:', { email: r.email, cpfDb, cpfNormalizado, status: r.status, match });
      return match;
    });

    if (!revendedora) {
      console.log('[NEXUS] Revendedora local nao encontrada ou inativa');
      return res.status(401).json({ error: 'Email ou CPF invalidos ou conta inativa' });
    }
    ...
    // 2. Buscar credenciais do admin (Opcional - Pode estar no record da revendedora ou em app_settings)
    let projectName = 'Plataforma';
    let adminCredentials = null;

    try {
      const { rows: creds } = await pool.query(
        'SELECT * FROM admin_supabase_credentials WHERE tenant_id = $1',
        [adminId]
      );
      if (creds.length > 0) {
        adminCredentials = creds[0];
        projectName = adminCredentials.project_name || 'Plataforma';
      }
    } catch (err) {
      console.warn('[NEXUS] Erro ao buscar credenciais do admin localmente');
    }

    // 3. Criar sessão com dados do tenant (dois níveis de isolamento)
    ...
    // 🔐 NÍVEL 2: identidade única da pessoa física (UUID por CPF)
    req.session.resellerId = revendedora.id;
...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    if (!nome || !email || !cpf) {
      return res.status(400).json({ error: 'Campos obrigatorios: nome, email, cpf' });
    }
    ...
    const { rows: inserted } = await pool.query(
      `INSERT INTO revendedoras (admin_id, nome, email, cpf, telefone, status, company_slug)
       VALUES ($1, $2, $3, $4, $5, 'pendente', $6)
       ON CONFLICT (email) DO NOTHING
       RETURNING id`,
      [adminId, nome, email.toLowerCase().trim(), cpfNormalizado, telefone || null, req.session.companySlug || null]
    );

    const data = inserted[0];
    if (!data) {
      return res.status(400).json({ error: 'Já existe uma revendedora com este email.' });
    }
...
router.get('/admin/list', async (req: Request, res: Response) => {
  try {
    ...
    const { rows: data } = await pool.query(
      `SELECT id, nome, email, cpf, telefone, status, comissao_padrao, created_at 
       FROM revendedoras 
       WHERE admin_id = $1 
...
router.patch('/admin/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    if (updated.length === 0) {
      return res.status(404).json({ error: 'Revendedora nao encontrada' });
    }
...
router.patch('/admin/:id/comissao', async (req: Request, res: Response) => {
  try {
    ...
    const { rows: updated } = await pool.query(
      `UPDATE revendedoras 
       SET comissao_padrao = $1, updated_at = NOW() 
       WHERE id = $2 AND admin_id = $3
       RETURNING *`,
      [comissao, id, adminId]
    );
